...
def parse(s):
    ...
    #thre parse call should have sused all the tokens
...
def parse_tokens(tokens):
    ...
    if is_int(start):
        ...
        (varname,tokens) = parse_tokens(tokens[1:]) # get the variable name
        expect(tokens[0], "=")
        (child,tokens) = parse_tokens(tokens[1:])
        return (Stmt(varname,child),tokens)
    else:
        check(start.isalpha(), "Varibale names must be alphabetic")
